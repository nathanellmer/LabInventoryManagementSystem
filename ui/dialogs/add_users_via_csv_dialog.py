...
from core.db_get_functions import get_users_db_fieldnames

class AddUsersCSVDialog(QDialog):
    def __init__(self):
        super().__init__()

        # Set the window size and title
        self.resize(400, 400)
        self.setWindowTitle("Add Users via CSV to Database")

        # Initialise the AddUsersCSVDialog layout and its margins (left, top, right, bottom)
        add_users_csv_dialog_layout = QVBoxLayout()
        add_users_csv_dialog_layout.setContentsMargins(20, 20, 20, 20)

        # Add the header widget to the add item dialog layout
        add_users_csv_dialog_layout.addWidget(FormHeaderWidget("Add Users via CSV to Database:"))

        # Add stretch
        add_users_csv_dialog_layout.addStretch()

        # Add the CSV panel widget
        self.csv_panel = CSVPanelWidget(self.btn_blank_csv, self.btn_upload_csv)
        add_users_csv_dialog_layout.addWidget(self.csv_panel, stretch=1)

        # Add spacing
        add_users_csv_dialog_layout.addSpacing(10)

        # Add a proceed button
        add_users_csv_dialog_layout.addWidget(MainMenuButton("Proceed", self.btn_proceed), alignment=Qt.AlignCenter)
        ...
        add_users_csv_dialog_layout.addWidget(FormFooterWidget())

        # Set the main layout for the window
        self.setLayout(add_users_csv_dialog_layout)


    def btn_blank_csv(self):
        # Create a blank CSV template with the appropriate headers
        headers = get_users_db_fieldnames()
        with open("user_csv_template.csv", mode="w", newline="") as file:
        ...
        if file_dialog.exec_():
            ...
            with open(selected_file, mode="r") as file:
                ...
                for row_idx, row in enumerate(reader):
                    ...
                    self.csv_users.append(row)


    def btn_proceed(self):
        # Loop through items to add
        for item in self.csv_users:
            ...
            dialog_close = add_user_to_db(field_values, False)

            # Close the dialog
            if dialog_close:
                self.close()

        # Show a message dialog to confirm the users have been added
        msg_dialog = MsgDialog("Users Added", "The users from the CSV file have been added to the database.", "OK")
        msg_dialog.exec_()
